...
!Copyright (C)  2023 A.J. Koning
  subroutine write_covariance(indent,reaction,MFa,MTa,MFb,MTb,N)
!
! +---------------------------------------------------------------------
! | Author: Arjan Koning
! | Date  : July 15, 2023
! | Task  : Write output block for covariance data
! +---------------------------------------------------------------------
!
! ****************** Declarations **************************************
!
...
!
  ...
  if (MFb == 0 .and. MTb == 0) then
    call write_char(id2,'class','variance')
    return
  endif
  if (MFa == MFb .and. MTa == MTb) then
    call write_char(id2,'class','in-channel covariance')
  else
    call write_char(id2,'class','cross-channel covariance')
    call write_char(id2,'reaction',reaction)
    call write_endfMFMT(id2,MFb,MTb)
  endif
  if (N > 0) call write_integer(id2,'number of random samples',N)
  return
  end
!Copyright (C)  2023 A.J. Koning
